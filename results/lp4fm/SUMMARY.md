...
**The `masked_best` and `name_only` columns involve no model.** A character
n-gram classifier is fitted on the
...
- Typologically close pairs (no Python), n=6: baseline 0.952, probe 0.893.
- Pairs involving Python, n=12: baseline 0.785, probe 0.887.
- The baseline tracks typological distance; the probe does not. The model's
  contribution is largest exactly where surface similarity is least, and near
  zero where n-grams already saturate.

The split is exhaustive: the baseline wins all 6 cells of the
typologically close pair, the probe wins all 12 cells involving Python,
and every gap clears its own probe rho.

Renaming conditions C1/C2/C4 are NOT in this table -- they were computed on the
earlier uncapped sample and are kept in `summary_renaming_uncapped.csv`.

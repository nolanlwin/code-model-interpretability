...
after the same intervention in two models. On Python, boolean occurrence type
is decodable at $0.981$--$0.988$ macro-F1, but a masked source-line classifier
reaches $0.983$ without a language model. The retained summaries do not support
uncertainty on that difference, so they do not establish a probe advantage
beyond local syntax. Class or structure labels remain decodable after
...
Boolean & 0.981--0.988 & surface 0.983 &
No resolved evidence beyond masked local source in Python \\
...
The committed probe and surface exports retain aggregate scores but not aligned
predictions from both predictors. Consequently, these small differences have
no paired interval and cannot support equivalence or superiority. Five
renaming conditions also change refit performance by at most $0.013$. The
supported claim is narrow: boolean occurrence type is linearly decodable after
the tested renames, while masked local syntax is a sufficient alternative
explanation for the high Python score. The cross-language runs extend
...
dissociation. A class-stratified boolean sample could resolve a consistent
probe advantage over the masked source line. An all-layer class-reference
patch could reveal a causal site outside the probe-selected layer. A
...
resulting difference. The boolean probes and masked-line classifier share a
population, but aligned predictions were not retained; a largest aggregate gap
of $0.005$ therefore supports neither probe superiority nor equivalence.
...
Three probes compared with the same masked-line classifier &
Not resolved. Retain paired predictions and test a declared margin \\
...
collectively, but not a complete $3\times5\times7$ design. Three-model
replication applies to boolean and class-reference; iterator uses two models,
and accumulator and index use one. Language coverage and extractors differ by
role.
...
lacks matched surface and control-task baselines. Boolean's local labels are
nearly surface-predictable; paired predictions were not retained, so
equivalence is untested. Class-reference patching covers one model, layer,
token site, and readout; stopping after the Qwen null was not externally
registered in advance.

Except for the reported patch, probes remain correlational. Mean pooling
directly exposes identifiers, while refitting tests recoverability rather than
fixed-direction invariance. Renames cover selected lexical alternatives;
cross-language transfer may reflect shared syntax, names, or extractor
artifacts. Hence the paper reports a matrix, not a conclusion that models
represent abstract variable roles.

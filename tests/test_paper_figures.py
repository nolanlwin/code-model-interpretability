...
def run() -> int:
    ...
    if MECH.exists() and MECHABL.exists():
        ...
        checks += [
            ...
            ("coefficient agreement DOES predict transfer (r = +0.89)",
             f'{corr([f(r,"coef_agreement") for r in mech], f1s):+.2f}' == "+0.89"),
            ("sign-flip mass predicts inversely (r = -0.82)",
             f'{corr([f(r,"sign_disagreement_mass") for r in mech], f1s):+.2f}' == "-0.82"),
            ("~70% of mass survives even in the worst pair",
             all(0.69 <= f(r, "surviving_mass") <= 0.87 for r in mech)),
            ("close pair agreement 0.91-0.93",
             all(0.91 <= f(r, "coef_agreement") <= 0.93 for r in near_)),
            ("python-crossing agreement 0.37-0.62",
             all(0.37 <= f(r, "coef_agreement") <= 0.62 for r in far_)),
            ("python-crossing sign-flip 28-40%",
             all(0.279 <= f(r, "sign_disagreement_mass") <= 0.40 for r in far_)),
            ("close-pair sign-flip about 6.5%",
             all(0.06 <= f(r, "sign_disagreement_mass") <= 0.07 for r in near_)),

...
$0.272$ yet increase accumulator F1 by $0.079$. Across three models, refit
iterator probes exceed a name-only baseline of $0.918$. On Python, boolean occurrence type is decodable at $0.981$--$0.988$ macro-F1,
...
$0.017$. An iterator name-only classifier reaches $0.918$ and does not match
the probe. Class or structure probe performance remains stable under tested renames
...
occurrences, uses problem-grouped splits, and predicts usage type. Iterator and
the class or structure pipeline use token labels, problem groups, and five seeds. The modern pipelines select
...
class or structure null permutes labels within programs and iterator uses a
...
Boolean, iterator, and class or structure experiments use problem-grouped splits,
...
Iterator provides a separate refit-robust result. Across the three models,
baseline F1 is $0.976$, $0.978$, and $0.987$. The widest BCa $95\%$ interval
is $[0.964,0.982]$, and Hewitt selectivity is $+0.526$ to $+0.552$. Misleading
names increase F1 by $0.012$--$0.023$, although the selected probes are at
...
reaches $0.918$, leaving an unpaired probe gap of $0.058$--$0.069$.
Python-trained transfer ranges are $0.934$--$0.981$, $0.898$--$0.923$, and
$0.887$--$0.938$. These results support refit recoverability and a probe
advantage over name-only form, not feature invariance or abstraction.
Exploratory iterator patching appears in the appendix and is not the class or
structure causal gate.
...
were run for each role. Boolean's matched surface comparator finds masked
local syntax sufficient. Iterator has a name-only comparator, but the
probe-minus-surface gap is not a paired clustered interval. Only class or
structure has a causal gate, which the intervention fails at the selected site.
...
\midrule
Decodability & $\circ$ & $\circ$ & $\checkmark$ & $\checkmark$ & $\checkmark$ \\
Randomized-label control & NR & NR & $\checkmark$ & $\checkmark$ & $\checkmark$ \\
Rename intervention & $\circ$ & $\circ$ & $\checkmark$ & $\checkmark$ & $\checkmark$ \\
Surface comparator & NR & NR & $\checkmark$ & $\checkmark$ & NR \\
...
\small
\setlength{\tabcolsep}{3.5pt}
\caption{Scientific status of the main interpretations. ``Open'' denotes a
missing matched comparison.}
\label{tab:status}
\begin{tabular}{p{0.23\linewidth}p{0.32\linewidth}p{0.34\linewidth}}
\toprule
Interpretation & Evidence & Status and next test \\
\midrule
Legacy roles may use different cues &
Opposite legacy accumulator and index deltas &
Exploratory. Rerun with problem groups and intervals \\
Iterator refit performance survives misleading names &
Three-model F1 $0.976$--$0.987$, name-only $0.918$ &
Supported as recoverability. Pair probe and surface predictions \\
Boolean exceeds Python local syntax &
Three-model paired reruns over $915$ problems &
Not supported. $95\%$ intervals exclude advantages above $0.014$--$0.017$ \\
Class refit performance survives renaming &
Three-model selectivity and rename deltas near zero &
...
Not supported at Qwen layer 18. Run an exploratory all-layer sweep \\
Cross-language role abstraction &
Strong transfer with uneven coverage &
...
the per-role coverage. Legacy accumulator-index results lack problem-grouped
splits and scope-aware renaming. Iterator lacks a paired probe-minus-surface
interval. Boolean labels are nearly predictable from local form, with paired
$95\%$ upper endpoints at most $0.017$. Class or structure patching covers one
model, layer, site, and readout, and stopping after the Qwen null was not
registered in advance. Other probes remain correlational, and transfer may
reflect shared syntax, names, or extractor artifacts.

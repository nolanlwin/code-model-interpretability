...
intervals excluding any probe advantage above $0.014$--$0.017$. This is a
resolved negative. Class or structure labels remain decodable after
...
imply clearing another. Reporting which of the five a result has cleared, and
which remain untested, is what makes a probing claim falsifiable.
\end{abstract}
...
controlled boolean and class-reference
...
$0.017$. Class-reference probe performance remains stable under tested renames
...
class/structure pipeline uses token labels, problem groups, and five seeds. The modern pipelines select
...
controls each role has cleared.}
\label{tab:scope}
...
Iterator & Coder-1.5B, StarCoder2-7B & Python, Java, JavaScript, PHP, C++, C, C\# \\
...
class/structure null permutes labels within programs. Activation patching asks
...
problem-clustered probe-minus-surface deltas over $915$ problem clusters are
$-0.002$ $[-0.017, 0.014]$, $-0.003$ $[-0.037, 0.014]$, and $+0.002$
$[-0.016, 0.017]$. Every interval covers zero, and each excludes a
...
sharper. Boolean occurrence type is decodable after
...
The class-reference and boolean experiments use different controls. Across all
...
No row entails the next, and Table~\ref{tab:controls} records what each role
has cleared. Only boolean has a matched surface comparator, under which masked
local syntax remains sufficient, and only class or structure has a causal
gate, which the intervention fails at the selected site.

\begin{table}[t]
\centering
\small
\caption{Controls each role has cleared. Check marks run and cleared, cross
marks run and not cleared, circle marks a legacy-protocol result awaiting
replication, dash marks not run.}
\label{tab:controls}
\begin{tabular}{lccccc}
\toprule
Control & Accum. & Index & Iterator & Boolean & Class \\
\midrule
Decodability & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ \\
Probe capacity & -- & -- & -- & $\checkmark$ & $\checkmark$ \\
Cue sensitivity & $\circ$ & $\circ$ & $\checkmark$ & $\checkmark$ & $\checkmark$ \\
Surface sufficiency & -- & -- & -- & $\times$ & -- \\
Causal relevance & -- & -- & -- & -- & $\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
A useful contrast varies one specified factor and quantifies uncertainty in
the resulting difference. The boolean probes and masked-line classifier use the
same evaluation population. The paired rerun provides an interval for their
difference and shows that no model's probe advantage exceeds $0.017$. This
converts an unresolvable aggregate gap of $0.005$ into a bounded negative.
...
gate, although noising has a positive interval. The result rejects the
substantive bidirectional claim at that site, not all causal involvement.
...
the per-role model, language, and extractor coverage.
...
artifacts record only aggregate scores. Boolean labels are nearly predictable from local form, with paired intervals
bounding any advantage below $0.017$. Class-reference patching covers one model, layer, site, and readout, and

"""
Per-site boolean-flag variable occurrences for Ruby (tree-sitter).
"""
...
from ruby_boolean_flag_roles import (
    RubyFlagHit,
    collect_flag_hits,
    collect_return_hits,
)
...
def enclosing_statement_type(parents: dict[Node, Node | None], node: Node) -> str | None:
    skip = frozenset({"program", "method", "body_statement"})
    cur = parents.get(node)
    while cur is not None:
        if cur.type in {"if", "while", "assignment", "return"}:
...
def _is_indexing_name(node: Node, parents: dict[Node, Node | None]) -> bool:
    p = parents.get(node)
    return p is not None and p.type in {"call", "element_reference"}
...
def iter_hit_names(h: RubyFlagHit) -> list[Node]:
...
def occurrence_rows_from_ruby_code(
    code: str,
    *,
    repo: str | None = None,
    path: str | None = None,

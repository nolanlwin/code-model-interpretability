"""
Per-site boolean-flag variable occurrences for JavaScript (tree-sitter).
"""
...
from javascript_boolean_flag_roles import (
    JavaScriptFlagHit,
    collect_flag_hits,
    collect_return_hits,
)
...
def _is_indexing_name(node: Node, parents: dict[Node, Node | None]) -> bool:
    p = parents.get(node)
    return p is not None and p.type in {"subscript_expression", "member_expression"}
...
def iter_hit_names(h: JavaScriptFlagHit) -> list[Node]:
...
def occurrence_rows_from_javascript_code(
    code: str,
    *,
    repo: str | None = None,
    path: str | None = None,
